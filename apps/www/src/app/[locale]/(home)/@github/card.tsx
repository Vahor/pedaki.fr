...
const StatusIcon = {
	...
	CLOSED_NOT_PLANNED: (
		<svg
			...
			xmlns="http://www.w3.org/2000/svg"
			role="img"
			aria-label="Issue status: CLOSED_NOT_PLANNED"
		>
			<path
				fill="currentColor"
				stroke="currentColor"
	...
	CLOSED_COMPLETED: (
		<svg
			...
			xmlns="http://www.w3.org/2000/svg"
			role="img"
			aria-label="Issue status: CLOSED_COMPLETED"
		>
			<path
				fill="currentColor"
			...
			<Card className="cursor-pointer bg-white focus-within:border-primary-base hover:border-primary-base">
				<CardContent className="space-y-2 pb-3 pt-6">
							...
							<TooltipContent className="max-w-xs">
								<span
									...
									dangerouslySetInnerHTML={{
										__html: issue.repository.descriptionHTML,
									}}
								></span>
							</TooltipContent>
						</Tooltip>
					...
					<div className="flex flex-row items-baseline gap-2">
						...
						<div>
							...
							></span>
							<span className="ml-1 text-sub-sm text-soft">
								#{issue.number}
							</span>
						</div>
					</div>
					<div className="flex flex-row items-center gap-2">
						{issue.labels.nodes.map((label) => {
							...
							const badgeComponent = (
								<Badge
									className={cn("border-transparent text-sub", {
										"text-white": dark,
									})}
							...
							return (
								<Tooltip key={label.name}>
									<TooltipTrigger className="text-soft">
										{badgeComponent}
									</TooltipTrigger>
									<TooltipContent className="max-w-xs">
				...
				<CardFooter className="border-t py-2">
					<div className="flex items-center gap-2">
						...
						<span className="text-sub-xs text-soft">
							Ouvert par {issue.author.name}
						</span>
					</div>
				</CardFooter>
			</Card>
