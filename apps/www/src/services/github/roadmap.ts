...
const fetchIssues = async (owner: string, name: string): Promise<Issue[]> => {
	...
	data.repository.issues.nodes = data.repository.issues.nodes.filter(
		(issue) => {
			if (
				ignoredUsers.includes(issue.author.name) ||
				ignoredUsers.includes(issue.author.login)
			) {
				return false;
			}

			if (
				ignoredLabels.some((label) =>
					issue.labels.nodes.some((node) => node.name === label),
				)
			) {
				return false;
			}
	...
	for (const issue of data.repository.issues.nodes) {
		...
		issue.repository.descriptionHTML = sanitizeHtml(
			issue.repository.descriptionHTML,
		);
		...
		async () => {
			...
			const issues = await Promise.all(
				repositories.map((repository) =>
					fetchIssues(repository.owner, repository.name),
				),
			);
